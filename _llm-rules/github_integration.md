...
## 運用手順
1. **原則**: IssueやPRにラベルは付与しない。ラベルが必要な場合は開発者が手動で付与する。
2. **必須**: descriptionの編集や、関連ブランチでの開発進行中に内容や状況が変化した場合も、ステータス（open/closed）の変更をAIが能動的に提案・実行する。
...
2. エラーや制約で実行不可の場合、一時ファイル方式に自動切り替え
3. いずれの方式も記録し、今後の改善に活用する
