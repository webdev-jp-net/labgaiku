...
- **Issue/PRのdescriptionやコメント**で改行を含むMarkdownを記述する際は、**printf + パイプ方式**を優先採用する。
- **コミットメッセージ**では、**HEREDOC方式**を使用する。
- printf + パイプ方式で差支えがある場合は、**一時ファイル方式**をバックアップとして使用する。
...
```bash
# mkdir -p .claude/tmp で置き場を用意し、Writeツールで .claude/tmp/comment.md に本文を書いたうえで実行する
...
1. まずprintf + パイプ方式で実行を試みる
2. エラーや制約で実行不可の場合、一時ファイル方式に切り替える

## リポジトリの初期化

コミットが1つも無いリポジトリでは、以下のブランチ運用がまだ成立しない。`develop` が存在せず、
featureからのPRも作れないため、一度だけ次の手順で立ち上げる。
...
ブランチはローカルにだけ作る。リモートへのpushはPR作成時に行う。
...
3. **ブランチ作成**: ローカルに作成する
4. **コミット**: 作業内容をコミット
5. **PR作成**: `git push -u origin [ブランチ名]` でリモートへ反映し、PRを作成する
6. **作業完了後**: Merge完了でIssue自動クローズ
