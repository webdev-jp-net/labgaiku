...
## 概要
このドキュメントは、`tamsan-lab-reports` プロジェクトで用いる名称を統一するための辞書です。

## プロジェクト名称
- **日本語名称**: Labが行く
- **英字表記スラグ**: `tamsan-lab-reports`
...
| 日本語 | コード表記 | 説明 | 使用例 |
|--------|------------|------|--------|
| レポート | report | microCMSの`report`コンテンツ | APIレスポンス、型定義 |
| 公開範囲 | visibility | `secret`/`limited`/`public` | フィルタリング、UI表示 |
| ゲスト | guest | レポート対象者 | UI表示 |
| 開催日 | date | ラボ企画の開催日 | UI表示 |
| 本文 | content | レポート本文（HTML） | 表示、サニタイズ |
...
## API / 型命名
- microCMS のレスポンス型は `Report` など PascalCase で定義
