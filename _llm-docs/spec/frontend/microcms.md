...
判定ロジックは `src/lib/permissions.ts` の `canViewReport(report, session)` に集約。
...
- `src/app/(authenticated)/app/`
  - サーバーコンポーネントで`getReportList`などを呼び出し、データを`_parts`の表示コンポーネントへ渡す
- `src/app/(authenticated)/app/_parts/`
  - クライアントコンポーネント側で描画とユーザー操作を担当

## 取得処理の方針

1. App Routerのサーバーコンポーネント（例: `app/page.tsx`や`(authenticated)/app/page.tsx`）で`getReportList()`を呼び出す。
