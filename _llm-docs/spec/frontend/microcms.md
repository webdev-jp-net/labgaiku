...
- `secret`: 制作途中の下書き。`allowList`に該当するログインユーザのみ一覧で通常表示+リンク有効・詳細閲覧可。それ以外（未認証/非該当の認証ユーザ）は一覧から完全除外、詳細URL直アクセス時はログイン促しViewを表示
- `limited`: 限定公開。一覧ではタイトル/ゲストを伏せ字（全角アスタリスク`＊＊＊＊＊`）+非リンクで表示し、日付のみは生で表示。`allowList`に該当するログインユーザのみ伏せ字解除+リンク有効、詳細閲覧可。非該当ユーザの詳細URL直アクセス時はログイン促しViewを表示
...
- `src/app/(contents)/interview/_parts/`/`src/app/(contents)/interview/[slug]/_parts/`
  - View（`view.tsx`）とフック（`use*.ts`）に責務を分け、繰り返しUIは`components/`配下のparts-componentに切り出す

## 取得処理の方針

- App Routerのサーバーコンポーネントで`getInterviewList`/`getInterviewById`を呼び出す
- 一覧取得時は`orders: '-date'`を渡し、開催日が新しい順に並んだ状態で取得する
- 取得結果は`_parts`のView/フックに渡し、表示用の派生値はフック側でまとめる
...
- サニタイズ処理は`src/lib/sanitize.ts`の`sanitizeHtml`に集約し、詳細ページでは`useInterviewDetail`フック内で適用してからViewに渡す（`view.tsx`から`sanitizeHtml`を直接呼ばない）
- 描画は`dangerouslySetInnerHTML`を経由する

## 目次（TOC）の生成

- 詳細ページのサーバーコンポーネントで`cheerio`を用いて`interview.content`から`h1`〜`h3`の`id`とテキストを抽出し、配列としてViewに渡す
- microCMSリッチエディタが見出しタグに自動付与する`id`を利用してページ内アンカーリンクを構成する
