...
- `interview.allowList`: `secret` / `limited` 記事の閲覧許可リスト（メールアドレスとドメインを 1 行 1 件で混在可、`@` の有無で自動判定）

判定の詳細は `src/lib/permission.ts` を参照。

## ルート保護

ミドルウェアによる一括保護は行わない（`src/middleware.ts` は不要のため未配置）。各ページ／詳細ルートは個別に `canViewInterview` で判定し、権限がない場合は `notFound()` を返す。
